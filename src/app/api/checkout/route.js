...
export async function POST(request) {
  try {
    ...
    if (isPastEvent(plan.date)) {
      return NextResponse.json(
        { error: "Este plan ya ha finalizado y no admite reservas" },
        { status: 400 },
      );
    }
    ...
    if (unitPrice === 0) {
      ...
      try {
        ...
        console.error(
          "Failed to send confirmation email automatically:",
          emailError,
        );
    ...
    await supabaseAdmin.from("reservations").insert({
      ...
      total_amount: Math.round(
        unitPrice * 100 * quantity + parseFloat(shippingCost || 0) * 100,
      ),
