...
export default async function PlanDetailPage({ params }) {
  ...
  const hasTicketsOrLists =
    plan.tickets?.length > 0 || plan.guestList?.length > 0;
  if (plan.type === "evento" && hasTicketsOrLists) {
    return (
      <>
        ...
        />
        <EventLayout plan={plan} relatedPlans={relatedPlans} isPast={isPast} />
      </>
    );
  }
...
/* =============================================
   ...
   ============================================= */
function EventLayout({ plan, relatedPlans, isPast = false }) {
    ...
    <div className={styles.eventPage}>
      <div className="container">
        ...
        {isPast && (
          ...
          >
            <strong>Este evento ya ha finalizado.</strong> Las entradas ya no
            están disponibles.
          </div>
        )}
            ...
            {plan.tickets.map((ticket, i) => (
              ...
              >
                <div className={styles.ticketInfo}>
                  ...
                  {ticket.capacity > 0 && !ticket.soldOut && (
                    <div className={styles.ticketRemaining}>
                      ⏱️ Quedan{" "}
                      {Math.max(0, ticket.capacity - (ticket.spots_taken || 0))}{" "}
                      plazas
                    </div>
                  )}
                </div>
                <div className={styles.ticketRight}>
                  ...
                  {isPast ? (
                    <span className={styles.soldOutBadge}>Finalizado</span>
                  ) : ticket.soldOut ? (
                      ...
                      {isPast ? (
                        ...
                        <a
                          href={`https://wa.me/34600000000?text=${encodeURIComponent(
                            `Hola! Quiero apuntarme a la lista "${list.name}" del plan "${plan.title}"`,
                          )}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className={styles.ticketBtn}
                          aria-label="Apuntarse a lista"
                          id={`list-${i}`}
                        >
                          →
                        </a>
        ...
        <section className={styles.eventSection}>
          <div className={styles.eventInfoGrid}>
            ...
            <div className={styles.eventInfoLeft}>
              <h3>Información</h3>
              <div className={styles.eventInfoText}>{plan.description}</div>
              ...
              {((plan.age_groups && plan.age_groups.length > 0) ||
                ...
                <div className={styles.eventBadgesSection}>
                  ...
                  {plan.etiquetas && plan.etiquetas.length > 0 && (
                    <div className={styles.eventBadgeGroup}>
                      <h4 className={styles.eventBadgeGroupTitle}>
                        🏷️ Etiquetas
                      </h4>
                      <div className={styles.eventBadgeList}>
                        {plan.etiquetas.map((etId) => {
                          ...
                          return (
                            <Link
                              href={`/planes/tag/${etId}`}
                              key={etId}
                              className={styles.eventEtiquetaBadge}
                            >
                              {et.emoji} {et.label}
                            </Link>
                          );
