...
export default function AdminReservasPage() {
  ...
  async function handleCancel(id) {
    if (
      !confirm(
        "¿Estás seguro de que deseas cancelar esta reserva y reembolsar el pago en Stripe (si aplica)?",
      )
    ) {
      return;
    }
      ...
      <div className={styles.pageHeader}>
        <div>
          ...
          <p className={styles.pageSubtitle}>
            {reservations.length} reservas · {(totalRevenue / 100).toFixed(2)}€
            ingresos totales
          </p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className={styles.emptyState}>
          ...
          <p className={styles.emptyDesc}>
            Las reservas aparecerán aquí cuando los clientes compren entradas
          </p>
        </div>
      ) : (
